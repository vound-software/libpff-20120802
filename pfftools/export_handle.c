     ...
     libcerror_error_t **error )
{
	...
	if( libpff_item_get_number_of_sets(
	     item,
	     &number_of_sets,
	     error ) != 1 )
	{
     ...
     libcerror_error_t **error )
{
	...
	if( attachment_filename_size < ( attachment_filename_index + 15 ) )
	{
...
/* TODO what about other types of message bodies */
		     ...
		     recovered_item_iterator++ )
		{
			     ...
			     error ) != 1 )
			{
				libcerror_error_set(
				 ...
				 LIBCERROR_ERROR_DOMAIN_RUNTIME,
				 LIBCERROR_RUNTIME_ERROR_FINALIZE_FAILED,
				 "%s: nable to free recovered item: %d.",
				 function,
				 recovered_item_iterator + 1 );
