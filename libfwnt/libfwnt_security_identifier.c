...
/* Initialize a security_identifier
     ...
     libcerror_error_t **error )
{
	...
	if( internal_security_identifier == NULL )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to create security_identifier.",
		 function );
	     ...
	     sizeof( libfwnt_internal_security_identifier_t ) ) == NULL )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to clear security_identifier.",
		 function );
...
/* Converts a class identifier stored in a byte stream into a runtime version
 * Returns 1 if successful or -1 on error
 */
