...
/* Initializes the index and its values
...
/* Frees the index and its values
 * Returns 1 if successful or -1 on error
 */
		     ...
		     entry_index++ )
		{
			     ...
			     error ) != 1 )
			{
				libcerror_error_set(
				 ...
				 "%s: unable to set index node entry: %" PRIu8 " as sub node.",
				 function,
				 entry_index );
			     ...
			     entry_index++ )
			{
				     ...
				     error ) != 1 )
				{
					libcerror_error_set(
					 ...
					 "%s: unable to append index node entry: %" PRIu8 " as sub node.",
					 function,
					 entry_index );
     ...
     libcerror_error_t **error )
{
	...
	if( io_handle == NULL )
	{
		libcerror_error_set(
		 ...
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
		 LIBCERROR_ARGUMENT_ERROR_INVALID_VALUE,
		 "%s: invalid index IO handle.",
		 function );

		return( -1 );
	}
	index = (libpff_index_t *) io_handle;
