     ...
     libcerror_error_t **error )
{
	...
	libpff_tree_node_t *sub_tree_node         = NULL;
	...
	if( item_tree_node->value == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
		 LIBCERROR_RUNTIME_ERROR_VALUE_MISSING,
		 "%s: invalid item tree node - missing value.",
		 function );

		return( -1 );
	}
	item_descriptor = (libpff_item_descriptor_t *) item_tree_node->value;
	...
	if( item_tree_node->number_of_sub_nodes > 0 )
	{
		sub_tree_node = item_tree_node->first_sub_node;

		for( sub_node_iterator = 0;
		     sub_node_iterator < item_tree_node->number_of_sub_nodes;
		     sub_node_iterator++ )
		{
			...
			sub_tree_node = sub_tree_node->next_node;
		}
	}
	return( result );
}
     ...
     libcerror_error_t **error )
{
	...
	if( item_tree_node->number_of_sub_nodes > 0 )
	{
		     ...
		     sub_node_iterator++ )
		{
			...
			if( sub_tree_node->value == NULL )
			{
				libcerror_error_set(
				 error,
				 LIBCERROR_ERROR_DOMAIN_RUNTIME,
				 LIBCERROR_RUNTIME_ERROR_VALUE_MISSING,
				 "%s: missing value for sub node: %d.",
				 function,
				 sub_node_iterator );

				return( -1 );
			}
			item_descriptor = (libpff_item_descriptor_t *) sub_tree_node->value;
     ...
     libcerror_error_t **error )
{
	...
	libpff_tree_node_t *parent_node                         = NULL;
...
/* TODO flag corrupt item tree */
	...
	else
	{
		...
		if( descriptor_index_value->identifier == descriptor_index_value->parent_identifier )
		{
			...
			result = libpff_tree_node_insert_node(
				  item_tree_root_node,
				  *root_folder_item_tree_node,
				  &libpff_item_descriptor_compare,
				  LIBPFF_TREE_NODE_INSERT_FLAG_UNIQUE_ENTRIES,
			          NULL );
		...
		else
		{
			...
			result = libpff_tree_node_insert_value(
				  parent_node,
				  (intptr_t *) item_descriptor,
				  &libpff_item_descriptor_compare,
				  LIBPFF_TREE_NODE_INSERT_FLAG_UNIQUE_ENTRIES,
     ...
     libcerror_error_t **error )
{
	...
	if( libpff_tree_node_append_value(
	     item_tree_node,
	     (intptr_t *) item_descriptor,
	     error ) != 1 )
	{
